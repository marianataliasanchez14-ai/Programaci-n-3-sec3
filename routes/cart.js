...
router.get('/', estaLogueado, (req, res) => {
    ...
    db.query(sql, [usuario_id], (err, results) => {
        ...
        const total = results.reduce((acc, item) => acc + parseFloat(item.subtotal), 0);
        res.json({ items: results, total_pagar: total.toFixed(2) });
    });
});
...
router.post('/checkout', estaLogueado, (req, res) => {
    ...
    const sqlObtenerItems = 'SELECT producto_id, cantidad FROM carrito WHERE usuario_id = ?';

    db.query(sqlObtenerItems, [usuario_id], async (err, items) => {
        ...
        try {
            ...
            for (const item of items) {
                const sqlUpdateStock = 'UPDATE productos SET stock = stock - ? WHERE id = ? AND stock >= ?';
...
router.delete('/:id', estaLogueado, (req, res) => {
    const sql = 'DELETE FROM carrito WHERE id = ? AND usuario_id = ?';
